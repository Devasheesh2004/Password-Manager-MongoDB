...
var _ = ["click", "mouseenter", "mouseleave"];
var b = "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype;
...
var I = class _I extends HTMLElement {
  ...
  connectedCallback() {
    if (this._root || this.createElements(), "lazy" === this.loading) {
      ...
      this.delayedLoading = (s2) => {
        for (const e3 of _) (t2 || this).removeEventListener(e3, i2);
      ...
      i2 = i2.bind(this);
      for (const e3 of _) (t2 || this).addEventListener(e3, i2);
  ...
  createElements() {
    if (this._root = this.attachShadow({ mode: "open" }), b) E || (E = new CSSStyleSheet(), E.replaceSync(v)), this._root.adoptedStyleSheets = [E];
    else {
  ...
  async createPlayer() {
    ...
    }), this.animationContainer.classList.add("ready"), this._isReady = true, this.dispatchEvent(new CustomEvent("ready"));
  }
  destroyPlayer() {
    this._isReady = false, this._loadedIconData = void 0, this._triggerInstance && (this._triggerInstance.onDisconnected && this._triggerInstance.onDisconnected(), this._triggerInstance = void 0), this._playerInstance && (this._playerInstance.disconnect(), this._playerInstance = void 0, this.animationContainer.classList.remove("ready"));
  }
  async loadIconData() {
    let e2 = this.iconData;
    if (!e2) {
      if (this.icon && _I._iconLoader) this._loadedIconData = e2 = await _I._iconLoader(this.icon);
      else if (this.src) {
  ...
  set icon(e2) {
    ...
    e2 && i(e2) ? (this._assignedIconData = e2, t2 !== e2 && (this.hasAttribute("icon") ? this.removeAttribute("icon") : this.iconChanged())) : (this._assignedIconData = void 0, e2 && "string" == typeof e2 ? this.setAttribute("icon", e2) : this.hasAttribute("icon") ? this.removeAttribute("icon") : t2 && this.iconChanged());
  }
  get icon() {
  ...
  set iconData(e2) {
    e2 !== this._assignedIconData && (this._assignedIconData = e2, this.iconChanged());
  }
  get iconData() {
...
var L = class {
  ...
  connect() {
    ...
    }), this._lottie.isLoaded ? (this._isReady = true, this.triggerEvent("ready")) : this._lottie.addEventListener("config_ready", () => {
      this._isReady = true, this.triggerEvent("ready");
    });
  }
  disconnect() {
    if (!this._lottie) throw new Error("Not connected player!");
    this._isReady = false, this._lottie.destroy(), this._lottie = void 0, this._colorsProxy = void 0, this._rawProperties = void 0;
  }
...
var T = [{ name: "mousedown" }, { name: "touchstart", options: { passive: true } }];
var P = class {
  player;
  element;
  targetElement;
  ...
  onConnected() {
    for (const e2 of T) this.targetElement.addEventListener(e2.name, this.onClick, e2.options);
  }
  onDisconnected() {
    for (const e2 of T) this.targetElement.removeEventListener(e2.name, this.onClick);
  }
  onClick() {
...
var q = class {
  ...
  constructor(e2, t2, i2) {
    this.player = e2, this.element = t2, this.targetElement = i2, this.observer = new MutationObserver((e3, t3) => {
      for (const t4 of e3) "attributes" === t4.type && ["sequence", "speed"].includes(t4.attributeName) && (this.reset(), this.step());
    });
  }
  onReady() {
    this.step();
...
function O(e2) {
  I.setPlayerFactory((t2, i2, s2) => new L(e2, t2, i2, s2)), I.defineTrigger("in", F), I.defineTrigger("click", P), I.defineTrigger("hover", A), I.defineTrigger("loop", x), I.defineTrigger("loop-on-hover", M), I.defineTrigger("morph", S), I.defineTrigger("boomerang", D), I.defineTrigger("sequence", q), I.defineTrigger("morph-two-way", D), customElements.get && customElements.get("lord-icon") || customElements.define("lord-icon", I);
}
export {
  D as Boomerang,
  P as Click,
